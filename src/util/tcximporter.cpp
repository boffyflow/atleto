// $Id: tcximporter.cpp 370 2010-12-14 23:23:50Z boffyflow $
...
void TcxImporter::loadDeviceData( Run *run, QFileInfo& fi)
{
    ...
    {
        QMessageBox::critical( 0, tr( "TCX open failed"), tr( "Could not open TCX file as read only."));
    ...
    {
        file.close();
        QMessageBox::critical( 0, tr( "TCX read failed"), tr( "Could not read or parse TCX file."));
    ...
    for( int i = 0; i < lapList.count(); i++)
    {
        ...
        if( !e.isNull())
        {
            ...
            if( split->distance() > TCX_MIN_SPLIT_DISTANCE)     // only add splits that are longer than TCX_MIN_SPLIT_DISTANCE
                run->addSplit( split);
        }
    }
}
